			...
			jQuery("#signin_student").submit(function(e){
					...
					$.ajax({
						...
						success: function(html){
							if(html == 'true') {
								$.jGrowl("Welcome to Study Spark", { header: 'Sign up Success' });
								setTimeout(function(){ window.location = 'dashboard_student.php' }, 2000);  
							} else if(html == 'exists') {
								$.jGrowl("This username is already taken.", { header: 'Username Exists' });
							} else if(html == 'false') {
								$.jGrowl("Student not found. Please check your details.", { header: 'Sign Up Failed' });
							}
						}	
					});
