<?php
include('session.php'); // Defines $session_id = $_SESSION['id']
include('header_dashboard.php');

$get_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
$session_id = $_SESSION['id'];  // logged-in teacher's user id
...
if (isset($_GET['delete_post_id'])) {
    ...
    mysqli_query($conn, "DELETE FROM discussion_reply WHERE discussion_post_id = '$delete_post_id'") or die(mysqli_error($conn));

    // Delete the post
    mysqli_query($conn, "DELETE FROM discussion_post WHERE discussion_post_id = '$delete_post_id'") or die(mysqli_error($conn));

    header("Location: discussion_teacher.php?id=$get_id");
    exit;
}
...
if (isset($_GET['delete_reply_id'])) {
    ...
    mysqli_query($conn, "DELETE FROM discussion_reply WHERE discussion_reply_id = '$delete_reply_id'") or die(mysqli_error($conn));

    $post_id = isset($_GET['post_id']) ? intval($_GET['post_id']) : 0;
    $location = "discussion_teacher.php?id=$get_id";
    if ($post_id > 0) $location .= "#post$post_id";
...
if (isset($_POST['new_post'])) {
    ...
    if ($hasContentOrMedia) {
        ...
        // Insert post with teacher_id set from session
        $query = "INSERT INTO discussion_post 
            (teacher_class_id, teacher_id, post_content,
             media_type_1, media_path_1,
             media_type_2, media_path_2,
             media_type_3, media_path_3)
            VALUES (
             '$get_id', '$session_id', '$content_escaped',
...
if (isset($_POST['new_reply'])) {
    ...
    if ($post_id > 0 && $hasContentOrMedia) {
        ...
        // Insert reply with teacher_id set from session
        $query = "INSERT INTO discussion_reply 
            (discussion_post_id, teacher_id, reply_content,
             media_type_1, media_path_1,
             media_type_2, media_path_2,
             media_type_3, media_path_3)
            VALUES (
             '$post_id', '$session_id', '$content_escaped',
             " . ($media_types[0] ? "'{$media_types[0]}'" : "NULL") . ",
             " . ($media_paths[0] ? "'{$media_paths[0]}'" : "NULL") . ",
             " . ($media_types[1] ? "'{$media_types[1]}'" : "NULL") . ",
             " . ($media_paths[1] ? "'{$media_paths[1]}'" : "NULL") . ",
        ...
        if (isset($_POST['ajax_request'])) {
            ...
            header("Location: discussion_teacher.php?id=$get_id#post$post_id");
            exit;
        }
    } else {
        if (isset($_POST['ajax_request'])) {
...
// Same media display function as before
...
// Display Delete button for teachers (can delete all posts and replies)
function displayDeleteButton($type, $id) {
    global $session_id, $conn;

    if ($type === 'post') {
        $check = mysqli_query($conn, "SELECT teacher_id FROM discussion_post WHERE discussion_post_id = $id");
        if ($check && mysqli_num_rows($check) > 0) {
            $row = mysqli_fetch_assoc($check);
            // Teachers can delete any post
            echo ' <a href="discussion_teacher.php?id=' . htmlspecialchars($_GET['id']) . '&delete_post_id=' . $id . '" onclick="return showDeleteConfirmation(\'post\', ' . $id . ');" class="twitter-delete-btn">Delete</a>';
        }
    } elseif ($type === 'reply') {
        $check = mysqli_query($conn, "SELECT teacher_id FROM discussion_reply WHERE discussion_reply_id = $id");
        if ($check && mysqli_num_rows($check) > 0) {
            $row = mysqli_fetch_assoc($check);
            // Teachers can delete any reply
            echo ' <a href="discussion_teacher.php?id=' . htmlspecialchars($_GET['id']) . '&delete_reply_id=' . $id . '" onclick="return showDeleteConfirmation(\'reply\', ' . $id . ');" class="twitter-delete-btn">Delete</a>';
        }
    }
}
?>
...
<div class="container-fluid">
    <div class="row-fluid">
        <?php include('discussion_link.php'); ?>
...
<script src="js/discussion_teacher.js"></script>
<style>
    ...
    window.showDeleteConfirmation = function(type, id) {
        ...
        if (type === 'post') {
            ...
            confirmBtn.onclick = function() {
                window.location.href = `discussion_teacher.php?id=${encodeURIComponent(window.location.search.match(/id=(\d+)/)[1])}&delete_post_id=${id}`;
            };
        } else if (type === 'reply') {
            message.textContent = 'Are you sure you want to delete this reply?';
            confirmBtn.onclick = function() {
                window.location.href = `discussion_teacher.php?id=${encodeURIComponent(window.location.search.match(/id=(\d+)/)[1])}&delete_reply_id=${id}`;
            };
        }
