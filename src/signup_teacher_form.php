			...
			jQuery("#signin_teacher").submit(function(e){
					...
					$.ajax({
						...
						success: function(html){
							if (html == 'true') {
								$.jGrowl("Welcome! Registration successful.", { header: 'Success' });
								setTimeout(function(){ window.location = 'dasboard_teacher.php'; }, 2000);
							} else if (html == 'exists') {
								$.jGrowl("Username already exists. Please choose another.", { header: 'Registration Failed' });
							} else if (html == 'false') {
								$.jGrowl("No matching teacher found. Please check your name and department.", { header: 'Registration Failed' });
							}
						}
