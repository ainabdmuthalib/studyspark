...
if (isset($_POST['add'])){
	...
	$check_query = mysqli_query($conn, "SELECT * FROM event WHERE event_title = '" . mysqli_real_escape_string($conn, $title) . "' AND date_start = '" . mysqli_real_escape_string($conn, $date_start) . "' AND date_end = '" . mysqli_real_escape_string($conn, $date_end) . "' AND teacher_class_id = '' ") or die(mysqli_error($conn));
	if (mysqli_num_rows($check_query) > 0) {
		// Duplicate found, show error
		echo '<script>alert("An event with the same title and dates already exists.");</script>';
	} else {
		...
		<table cellpadding="0" cellspacing="0" border="0" class="table" id="">
									...
									?>                              
										<tr id="del<?php echo $id; ?>">
										 ...
										 <a  class="btn btn-danger delete-event-btn" href="delete_event.php<?php echo '?id='.$id; ?>"><i class="icon-remove icon-large"></i></a>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    var deleteLinks = document.querySelectorAll('.delete-event-btn');
