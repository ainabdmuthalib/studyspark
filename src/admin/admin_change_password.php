...
<?php include('navbar.php'); ?>
    <div class="container-fluid">
        ...
        <?php include('sidebar_dashboard.php'); ?>
            <div class="span2">
                <!-- Optionally, add a sidebar or leave empty for admin -->
            </div>
            ...
            jQuery(document).ready(function(){
                ...
                jQuery("#change_password").submit(function(e){
                    ...
                    if (!validatePassword(new_password)) {
                        $.jGrowl("New password does not meet requirements", { header: 'Change Password Failed' });
                        return false;
                    }
                    // Validate match
                    if (new_password != retype_password) {
                        $.jGrowl("Passwords do not match", { header: 'Change Password Failed' });
                        return false;
                    }
                    ...
                    $.ajax({
                        ...
                        data: {
                            user_id: <?php echo json_encode($session_id); ?>,
                            new_password: new_password
                        },
                        success: function(response) {
                            if (response === 'success') {
                                $.jGrowl("Password changed successfully", { header: 'Success' });
                                jQuery('#change_password')[0].reset();
                                updateRequirementIcon('length', false);
                                updateRequirementIcon('case', false);
                                updateRequirementIcon('special', false);
                                checkPasswordMatch();
                                updateSubmitButton();
                            } else {
                                $.jGrowl("Failed to change password", { header: 'Error' });
                            }
                        }
                    });
                });
