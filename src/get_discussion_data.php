    ...
    for ($i = 1; $i <= 3; $i++) {
        ...
        if ($media_type == 'image') {
            ...
            if ($file_ext === 'pdf') {
                ...
                $mediaHtml .= '<br><a href="' . $media_path_esc . '" target="_blank" style="font-weight:bold; text-decoration:underline;">📄 ' . htmlspecialchars($file_name) . ' (Open PDF in new tab)</a>';
            } else {
                $icon = ($file_ext === 'docx') ? '📝' : (($file_ext === 'pptx') ? '📊' : '📁');
                $mediaHtml .= '<a href="' . $media_path_esc . '" target="_blank" style="font-weight:bold; text-decoration:underline;">' . $icon . ' ' . htmlspecialchars($file_name) . ' (Open in new tab)</a>';
            }
