document.addEventListener('DOMContentLoaded', function() {
    ...
    function appendPost(post) {
        ...
        postElement.innerHTML = `
            <div class="navbar navbar-inner block-header">
                ...
                <div>
                    <a href="discussion_teacher.php?id=${teacherClassId}&delete_post_id=${post.discussion_post_id}" 
                       onclick="return confirm('Are you sure you want to delete this post and all its replies?');"
                       class="btn btn-danger btn-small">Delete Post</a>
                </div>
            </div>
    ...
    function appendReply(reply) {
        ...
        replyElement.style.cssText = 'border-bottom: 1px solid #ddd; margin-bottom: 5px; padding-bottom: 5px; display:flex; justify-content:space-between; align-items:center;';

        replyElement.innerHTML = `
            <div>
                <strong>${DOMPurify.sanitize(reply.replier_name)}</strong> replied on ${reply.reply_date_formatted}:<br>
                <p>${DOMPurify.sanitize(reply.reply_content)}</p>
                ${reply.media_html}
            </div>
            <div>
                <a href="discussion_teacher.php?id=${teacherClassId}&delete_reply_id=${reply.discussion_reply_id}&post_id=${reply.discussion_post_id}" 
                   onclick="return confirm('Are you sure you want to delete this reply?');"
                   class="btn btn-danger btn-mini">Delete Reply</a>
            </div>
        `;
    ...
    function handlePostSubmit(e) {
        ...
        fetch('discussion_teacher.php?id=' + teacherClassId, {
            method: 'POST',
            body: formData
        })
    ...
    function fetchDiscussionData() {
        fetch(`get_discussion_data_teacher.php?id=${teacherClassId}&last_post_id=${lastPostId}&last_reply_id=${lastReplyId}`)
            .then(response => response.json())
            .then(data => {
                // Add new posts
