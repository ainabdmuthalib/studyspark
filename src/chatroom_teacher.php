...
include('header_dashboard.php'); // Include the header
...
if (isset($_GET['delete_message_id'])) {
    ...
    header("Location: chatroom_teacher.php?id=$get_id");
    exit;
}
...
// Handle new message submission by instructor
if (isset($_POST['new_message'])) {
    // Ensure required variables are set
    if (!isset($get_id) || !isset($session_id)) {
    ...
    if ($hasContentOrMedia) {
        ...
        // Since this page is for instructors only, we can just insert teacher_id and skip student_id
        $query = "INSERT INTO chat_message 
            (teacher_class_id, teacher_id, message_content,
...
<body>
<?php include('navbar_teacher.php'); // Sidebar and navbar ?>
<div class="container-fluid">
    <div class="row-fluid">
        <?php include('chatroom_link.php'); // Chatroom navigation ?>
                        ...
                        if (mysqli_num_rows($messages_q) == 0) {
                            ...
                            while ($message = mysqli_fetch_assoc($messages_q)) {
                                ...
                                if (isset($message['teacher_id'])) {
                                    ...
                                    $is_own_message = ($message['teacher_id'] == $session_id);
                                } elseif (isset($message['student_id'])) {
                                    $sender_name = getUserName($conn, $message['student_id']);
                                    $is_own_message = false; // Students can't send messages in teacher chatroom
                                }
                                $message_date = date('d M Y, H:i', strtotime($message['message_date']));
                                ?>
                                <div class="dm-message-row <?php echo $is_own_message ? 'dm-own' : 'dm-other'; ?>">
                                    ...
                                    <div class="dm-bubble">
                                        ...
                                        <div class="dm-meta-row">
                                            ...
                                            <a href="chatroom_teacher.php?id=<?php echo $get_id; ?>&delete_message_id=<?php echo $message_id; ?>" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (messageForm) {
        messageForm.addEventListener('submit', function(e) {
            ...
            // Submit form
            fetch('chatroom_teacher.php?id=<?php echo $get_id; ?>', {
                method: 'POST',
                body: formData
            })
            .then(response => {
    ...
    // Auto-refresh functionality (matching student version)
    const messageList = document.getElementById('message-list');
    let lastMessageId = 0;
    ...
    function appendMessage(message) {
        const isOwnMessage = message.teacher_id == <?php echo $session_id; ?>;
        ...
        const deleteBtn = `<a href=\"chatroom_teacher.php?id=<?php echo $get_id; ?>&delete_message_id=${message.message_id}\" onclick=\"return confirm('Delete for everyone');\" class=\"dm-delete-btn\" title=\"Delete message\"><i class=\"icon-trash\"></i></a>`;
    ...
    function fetchMessages() {
        fetch(`get_messages_teacher.php?id=<?php echo $get_id; ?>&last_message_id=${lastMessageId}`)
